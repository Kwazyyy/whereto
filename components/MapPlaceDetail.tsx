...
function DetailPhoto({ photoRef, alt }: { photoRef: string; alt: string }) {
  ...
  if (!url) return <div className="w-full h-full bg-[#1C2128] animate-pulse" />;
  return (
    <Image
      src={url}
      alt={alt}
...
function DetailContent({
  ...
  return (
    <>
      {/* Photo section */}
      <div className="relative h-48 w-full shrink-0 bg-[#1C2128]">
        {photos ? (
          <div
            ref={carouselRef}
          ...
          >
            {photos.map((ref, idx) => (
              <div key={idx} className="w-full shrink-0 snap-center h-full relative bg-[#1C2128]">
          ...
          <div className="w-full h-full bg-gradient-to-br from-[#1C2128] to-[#2d333f]" />
        )}
      ...
      <div className="p-5 pb-6">
        ...
        <div className="flex items-center gap-2 mt-1.5 text-sm">
          ...
          {place.price && (
            <>
              <span className="text-[#8B949E]">{place.price}</span>
              <span className="w-1 h-1 rounded-full bg-[#30363D]" />
            </>
          )}
          <span className="text-[#8B949E] capitalize">{place.type}</span>
        </div>
        {place.address && (
          <p className="text-[#8B949E] text-sm mt-1">{place.address}</p>
        )}
        ...
        {place.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-3">
            {place.tags.map((tag) => (
              <span key={tag} className="px-3 py-1 rounded-full bg-white/10 text-[#C9D1D9] text-xs font-semibold">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          {/* Hours */}
          <div className="bg-[#1C2128] rounded-lg p-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-[#8B949E] font-medium">Hours</p>
              <p className="text-sm font-semibold text-white">{place.openNow ? "Open" : "Closed"}</p>
            </div>
          </div>
          {/* Distance */}
          <div className="bg-[#1C2128] rounded-lg p-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-[#8B949E] font-medium">Distance</p>
              <p className="text-sm font-semibold text-white">{computedDistance ?? "—"}</p>
            </div>
          </div>
          {/* Price */}
          <div className="bg-[#1C2128] rounded-lg p-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-[#8B949E] font-medium">Price</p>
              <p className="text-sm font-semibold text-white">{place.price || "—"}</p>
            </div>
          </div>
          {/* Type */}
          <div className="bg-[#1C2128] rounded-lg p-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-[#8B949E] font-medium">Type</p>
              <p className="text-sm font-semibold text-white capitalize">{place.type}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full mt-4">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm cursor-pointer ${
              isSaved
                ? "border border-[#E85D2A]/40 bg-[#E85D2A]/10 text-[#E85D2A]"
                : "border border-[#30363D] text-white hover:border-[#E85D2A] hover:text-[#E85D2A] transition-colors duration-200"
            }`}
          >
            <svg
          ...
          <button
            className="w-12 h-12 flex items-center justify-center rounded-xl border border-[#30363D] text-[#8B949E] hover:text-[#E85D2A] hover:border-[#E85D2A] transition-colors duration-200 cursor-pointer"
          >
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-3 mt-3 text-[#8B949E] text-sm font-medium cursor-pointer hover:text-white transition-colors duration-200"
        >
          {detailsOpen ? (
            <>
              Show Less
          ...
          {detailsOpen && (
            ...
            >
              ...
              {place.friendSaves && place.friendSaves.length > 0 && (
                <div className="mt-4">
                  <h3 className="text-xs font-semibold tracking-wider text-[#8B949E] uppercase mb-3">Friends Love This</h3>
                  <div className="space-y-2">
                    {place.friendSaves.map((f: FriendSignal) => (
                      <div key={f.userId} className="flex items-center gap-3 p-3 bg-[#1C2128] rounded-xl">
                        {f.image ? (
                          <Image
                            src={f.image}
                            alt={f.name ?? ""}
                            width={28}
                            height={28}
                            className="w-7 h-7 rounded-full border-2 border-[#161B22] object-cover"
                            unoptimized
                          />
                        ) : (
                          <div className="w-7 h-7 rounded-full bg-[#E85D2A] text-[10px] font-bold text-white flex items-center justify-center border-2 border-[#161B22]">
                            {f.name?.[0]?.toUpperCase() ?? "?"}
                          </div>
                        )}
                        <span className="text-sm font-medium text-white">{f.name ?? "A friend"}</span>
                        <span className="ml-auto text-xs text-[#8B949E]">saved this</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Details grid */}
              <div className="mt-4">
                <h3 className="text-xs font-semibold tracking-wider text-[#8B949E] uppercase mb-3">Details</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-[#1C2128]">
                    ...
                    <div>
                      <p className="text-xs text-[#8B949E] font-medium">Noise Level</p>
                      <p className="text-sm font-medium text-white">Quiet</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-[#1C2128]">
                    <span className="w-2.5 h-2.5 rounded-full bg-green-500 shrink-0 ml-0.5" />
                    <div>
                      <p className="text-xs text-[#8B949E] font-medium">Busyness</p>
                      <p className="text-sm font-medium text-white">Not Busy</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-[#1C2128]">
                    ...
                    <div>
                      <p className="text-xs text-[#8B949E] font-medium">Outlets</p>
                      <p className="text-sm font-medium text-white">Plenty</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-[#1C2128]">
                    ...
                    <div>
                      <p className="text-xs text-[#8B949E] font-medium">Seating</p>
                      <p className="text-sm font-medium text-white">Long Stay</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Hours row */}
              <div className="mt-4 flex items-center gap-2.5 p-3.5 rounded-xl bg-[#1C2128]">
              ...
              {place.menuUrl && (
                <a
                  ...
                  className="mt-2 flex items-center gap-3 p-3.5 rounded-xl bg-[#1C2128] hover:bg-[#252D38] transition-colors cursor-pointer"
                >
              ...
              <Link
                ...
                className="mt-2 flex items-center gap-3 p-3.5 rounded-xl bg-[#1C2128] hover:bg-[#252D38] transition-colors"
              >
                <Camera size={18} className="text-[#E85D2A] shrink-0" />
                <span className="flex-1 text-sm font-medium text-white">
                  Community Photos
                  {place.communityPhotoCount != null && place.communityPhotoCount > 0 ? (
                    <span className="text-[#8B949E] font-normal"> ({place.communityPhotoCount})</span>
                  ) : (
                    <span className="text-[#8B949E] font-normal text-xs"> — Be the first!</span>
                  )}
                </span>
    ...
    <>
      ...
      <div className="hidden lg:block">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
          <motion.div
            className="bg-[#161B22] rounded-2xl border border-[#30363D] max-w-[540px] w-[95%] max-h-[85vh] overflow-y-auto scrollbar-none pointer-events-auto relative"
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-[#8B949E] hover:text-white transition-colors cursor-pointer"
            >
      ...
      <div className="lg:hidden">
        ...
        {/* Sheet */}
        <motion.div
          className={`fixed bottom-0 left-0 right-0 z-50 bg-[#161B22] rounded-t-2xl border-t border-[#30363D] overflow-y-auto scrollbar-none transition-[max-height] duration-300 ease-out ${detailsExpanded ? "max-h-[90vh]" : "max-h-[55vh]"}`}
          initial={{ y: "100%" }}
          animate={{ y: 0 }}
          exit={{ y: "100%" }}
        ...
        >
          {/* Handle bar */}
          <div className="flex justify-center pt-3 pb-2 sticky top-0 z-10 bg-[#161B22] rounded-t-2xl">
